...
DEFAULT_PDF = Path(
    ...
    "Member_Monday_Lift__Bedding__-_Master_Sheet_1a80.pdf"
)
...
SUPPLIER_INFO = {
    ...
    "20128": ("SLNY Home INC, DBA Spirit Linen", "Supplier Service Desk"),
    "231356": ("SHENZHENMENGMI TEXTILE CO.,LTD", "lance Zhou"),
    "231496": ("TEEMODERN, LLC dba Ethan Taylor", "Alex Sarkisian"),
    "24696": (
        "Daniel Linen/Home Sweet Home Dreams Inc.",
        "Supplier Service Desk",
    ),
    "313243": ("SHANGHAI DAYA IMPORT & EXPORT CO., LTD.", "Asia Supplier Service Desk"),
    "348967": ("Nanyang Dingjiayue Shangmao Youxiansi", "Jessica Gu"),
    "352358": ("Shenzhen Eden Grove Decoration Co., LTD", "Asia Supplier Service Desk"),
    "43431": ("ocean home fashion inc", "Asia Supplier Service Desk"),
    "63138": ("Changshu Youthsky Import Export Co.,Ltd", "Jessica Gu"),
}
...
def clean_sku(supplier_id: str, value: object) -> str:
    ...
    compact = "".join(char for char in text if char.isupper() or char.isdigit())
    compact = compact.replace("GJSSTS", "JSST")
...
def write_report(
    ...
    if source_totals and not USE_PDF_DISPLAYED_TOTAL_FOR_HEADLINE:
        ...
        qa_note = (
            ...
            f"{fmt_currency_2(sales_delta)} on Member Monday sales."
        )
    else:
        qa_note = (
...
- **Lead with the proof point:** {best_class.class_name} delivered the strongest class-level lift, showing that even specialized {CATEGORY_NAME.lower()} classes respond when promoted through the loyalty event.
...
"""
    (out_dir / "Member_Monday_Bedding_Loyalty_Lift_Case_Study.md").write_text(report)
